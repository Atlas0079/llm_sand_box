...
func get_environment_context() -> Dictionary:
	...
	context["location_connections_summary"] = ", ".join(connections.values().map(func(c): return c.get("target_location_name", "一条路"))) if not connections.is_empty() else "无"
...
func _format_connections(location: Location) -> Dictionary:
	...
	var formatted = {}
	var path_ids = location.get_all_path_ids()
	for path_id in path_ids:
		var path_resource = WorldManager.get_path_by_id(path_id)
		var target_location_id = location.connections.get(path_id)
		
		if is_instance_valid(path_resource) and target_location_id:
			# 获取目标地点的名字，让LLM更容易理解
			var target_location_node = WorldManager.get_location_by_id(target_location_id)
			var target_location_name = target_location_id # 默认为ID
			if is_instance_valid(target_location_node):
				target_location_name = target_location_node.location_name

			formatted[target_location_id] = {
				"target_location_name": target_location_name,
				"path_name": path_resource.path_name,
				"travel_time": path_resource.travel_time
			}
	return formatted
...
func _perceive_entities_in(location: Location) -> Array[Dictionary]:
	...
	# 遍历地点内的所有实体ID
	for entity_id in location.entities_in_location:
		var entity = WorldManager.get_entity_by_id(entity_id)
...
func _format_container_contents(container: ContainerComponent) -> Array[String]:
	...
	for item_id in container.get_all_item_ids():
		...
		if is_instance_valid(item_entity):
			contents.append(item_entity.entity_name)
	return contents